...
- SQL 源文件在 `sql/` 下；Supabase CLI / GitHub Actions 使用 `supabase/migrations/`，需要通过 `scripts/sync-supabase-migrations.ps1` 同步。
- 不要把 Supabase `service_role` 或其他 secret 写进仓库。
...
4. 如果涉及数据库，先改 `sql/` 里的迁移文件，再运行 `scripts/sync-supabase-migrations.ps1`；需要远端部署时，优先走 Supabase CLI / GitHub Actions 流程。
